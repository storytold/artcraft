        ...
        switch (token) {
          ...
          default: {
            if (token.includes("m_")) {
              ...
              // Add any missing userdata back to the object. Older
              // scene JSON may lack a `user_data` field; keep the
              // default `{}` from THREE.Object3D's constructor in
              // that case so the downstream `obj.userData.name = ...`
              // assignment (below) doesn't crash on undefined.
