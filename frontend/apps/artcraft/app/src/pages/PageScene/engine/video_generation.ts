...
export class VideoGeneration {
  ...
  async generateFrame() {
    if (!this.editor.generating_preview && this.editor.rawRenderer) {
      ...
      usePageSceneStore.getState().setPreviewSrc("");
      ...
      try {
        ...
        usePageSceneStore.getState().setPreviewSrc(url);
  ...
  async stopPlaybackAndUploadVideo() {
    ...
    if (ipa_image_token) {
      usePageSceneStore.getState().setGlobalIPAMediaToken(ipa_image_token);
    }
    ...
    const enqueue_studio_response = await this.videoAPI.EnqueueStudio({
      enqueueVideo: {
        ...
        global_ipa_media_token:
          usePageSceneStore.getState().globalIPAMediaToken ?? "",
        input_depth_file: "",
        input_normal_file: "",
        input_outline_file: "",
