...
After a successful image or video generation (`generate_image` or `generate_video`):

1. Poll `get_job_status` with the returned `job_token` until status is `complete_success`
2. Get result media token from job status
3. Retrieve media details with `get_media_file` to get the CDN URL
4. **ASK THE USER**: "Would you like me to open the image for you?"
5. If yes, use `Start-Process` (Windows) or `open` (macOS) to launch the image with the default system viewer
6. Always save downloaded files to a user-specified directory

**Never open images automatically without user consent.** Always confirm first.
