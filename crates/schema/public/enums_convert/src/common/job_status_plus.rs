use enums_api::common::job_status_plus::JobStatusPlus as Api;
use enums_db::common::job_status_plus::JobStatusPlus as Db;

pub fn job_status_plus_to_db(api_value: &Api) -> Db {
  match api_value {
    ...
    Api::CancelledBySystem => Db::CancelledBySystem,
  }
}

pub fn job_status_plus_to_api(db_value: &Db) -> Api {
  match db_value {
    ...
    Db::CancelledBySystem => Api::CancelledBySystem,
  }
}
...
mod tests {
  ...
  fn round_trip_db_to_api() {
    ...
    for variant in Db::iter() {
      let api = job_status_plus_to_api(&variant);
      let back = job_status_plus_to_db(&api);
  ...
  fn round_trip_api_to_db() {
    ...
    for variant in Api::iter() {
      let db = job_status_plus_to_db(&variant);
      let back = job_status_plus_to_api(&db);
      assert_eq!(variant, back);
    }
  }
}
