...
/// A pending KinoviWeb character job waiting for a result.
#[derive(Debug)]
pub struct PendingKinoviWebCharacterJob {
...
/// Returns all non-terminal KinoviWeb character jobs that have an associated character ID.
pub async fn list_pending_kinovi_web_character_jobs(pool: &MySqlPool) -> Result<Vec<PendingKinoviWebCharacterJob>, sqlx::Error> {
  ...
  let jobs = records
    ...
    .filter_map(|record| {
      let kinovi_character_id = match record.kinovi_character_id {
        Some(id) => id,
        None => {
          warn!("PendingKinoviWebCharacterJob has no kinovi_character_id, skipping");
          return None;
        }
      };

      Some(PendingKinoviWebCharacterJob {
