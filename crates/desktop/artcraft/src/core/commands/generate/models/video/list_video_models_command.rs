...
//!
//! Returns COMMAND-SPECIFIC types (never `artcraft_client` types directly). API-client
//! responses are mapped into local types whose enums carry an `Unknown(String)` catch-all,
//! so a newer server never breaks an older desktop build. The request is retried up to
//! {MAX_ATTEMPTS} times, and successful responses are cached in-memory for 60 seconds.
...
use artcraft_client::endpoints::omni_gen::models::video::omni_gen_list_video_models::{
  omni_gen_list_video_models, OmniGenListVideoModelsArgs, OmniGenVideoModelDetails, OmniGenVideoModelProviderDetails, OmniGenVideoProviderModelDetails, OmniGenVideoModelsResponse,
};
...
async fn fetch_with_retry(api_host: &ApiHost) -> Result<OmniGenVideoModelsResponse, String> {
  ...
  for attempt in 1..=MAX_ATTEMPTS {
    match omni_gen_list_video_models(OmniGenListVideoModelsArgs {
      api_host,
      maybe_creds: None,
      provider: None,
    })
