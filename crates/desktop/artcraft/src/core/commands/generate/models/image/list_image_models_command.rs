...
use artcraft_client::endpoints::omni_gen::models::image::omni_gen_list_image_models::{
  omni_gen_list_image_models, OmniGenListImageModelsArgs, OmniGenImageModelDetails, OmniGenImageModelProviderDetails, OmniGenImageProviderModelDetails, OmniGenImageModelsResponse,
};
...
async fn fetch_with_retry(api_host: &ApiHost) -> Result<OmniGenImageModelsResponse, String> {
  ...
  for attempt in 1..=MAX_ATTEMPTS {
    match omni_gen_list_image_models(OmniGenListImageModelsArgs {
      api_host,
      maybe_creds: None,
      provider: None,
    })
