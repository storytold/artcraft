...
use mysql_queries::queries::generic_inference::api_providers::kinovi_web::insert_generic_inference_job_for_kinovi_web_queue_with_apriori_job_token::{
  ...
  InsertGenericInferenceForKinoviWebWithAprioriJobTokenArgs, KinoviVersion,
};
...
pub struct InsertKinoviWebJobsArgs<'a, 'tx> {
...
pub struct InsertKinoviWebJobsResult {
  pub primary_job_token: InferenceJobToken,
  pub all_job_tokens: Vec<InferenceJobToken>,
}

/// Insert one KinoviWeb/Kinovi inference job per returned order_id.
/// Mirrors the video-side helper at
/// `omni_gen::generate::video::insert_db_job::insert_kinovi_web_jobs`.
pub async fn insert_kinovi_web_jobs(args: InsertKinoviWebJobsArgs<'_, '_>) -> Result<InsertKinoviWebJobsResult, CommonWebError> {
  let InsertKinoviWebJobsArgs {
    primary_order_id,
  ...
  for (i, order_id) in all_order_ids.iter().enumerate() {
    ...
    match db_result {
      ...
      Err(err) => {
        warn!("Error inserting kinovi_web inference job (image, order_id={}): {:?}", order_id, err);
