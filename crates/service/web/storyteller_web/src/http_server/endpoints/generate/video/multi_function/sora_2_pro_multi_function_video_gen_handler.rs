...
pub async fn sora_2_pro_multi_function_video_gen_handler(
...
) -> Result<Json<Sora2ProMultiFunctionVideoGenResponse>, AdvancedCommonWebError> {
  ...
  if let Some(image_url) = maybe_image_url {
    ...
    let i2v_request = EnqueueSora2ProImageToVideoRequest {
      image_url,
    ...
    let t2v_request = EnqueueSora2ProTextToVideoRequest {
      prompt: request.prompt.as_deref().unwrap_or("").to_string(),
      duration: Some(duration),
      aspect_ratio: Some(aspect_ratio),
      resolution: Some(resolution),
