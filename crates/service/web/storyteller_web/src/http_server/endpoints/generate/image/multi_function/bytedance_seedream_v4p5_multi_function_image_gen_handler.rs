...
pub async fn bytedance_seedream_v4p5_multi_function_image_gen_handler(
...
) -> Result<Json<BytedanceSeedreamV4p5MultiFunctionImageGenResponse>, AdvancedCommonWebError> {
  ...
  if let Some(input_image_urls) = image_urls.as_deref() {
    ...
    let t2i_request = EnqueueBytedanceSeedreamV4p5TextToImageRequest {
      prompt: request.prompt.clone().unwrap_or_default(),
      num_images: Some(num_images),
      image_size: Some(image_size),
      max_images: None,
  ...
  let prompt_result = insert_prompt(InsertPromptArgs {
    ...
    maybe_model_type: Some(CommonModelType::Seedream4p5),
