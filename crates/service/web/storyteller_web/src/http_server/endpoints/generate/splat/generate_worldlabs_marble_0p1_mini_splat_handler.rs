...
pub async fn generate_worldlabs_marble_0p1_mini_splat_handler(
  ...
  let cost = calculate_cost(WorldLabsModel::Marble0p1Mini, InputType::ImageNonPanorama);
  ...
  let prompt_result = insert_prompt(InsertPromptArgs {
    ...
    maybe_model_type: Some(ModelType::Marble0p1Mini),
  ...
  // We don't have the external operation_id yet — the worker will call the WorldLabs API.
  // For now, record the job as pending with an empty external ID.
