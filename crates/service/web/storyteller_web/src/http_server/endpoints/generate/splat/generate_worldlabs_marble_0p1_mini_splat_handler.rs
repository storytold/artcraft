...
) -> Result<Json<GenerateWorldlabsMarble0p1MiniSplatResponse>, AdvancedCommonWebError> {
  ...
  let world_prompt = if let Some(image_token) = &request.image_media_file_token {
    ...
    let cdn_url = image_urls.get(image_token)
      .ok_or_else(|| {
        ...
        AdvancedCommonWebError::server_error_with_message("Image token was not found in lookup results")
      })?;
