...
use enums_db::by_table::users::user_feature_flag::UserFeatureFlag as DbUserFeatureFlag;
use enums_convert::by_table::users::user_feature_flag::user_feature_flag_to_api;
    ...
    Some(session_data) => {
      ...
      if !session_data.is_banned {
        ...
        user_info = Some(SessionUserInfo {
          ...
          can_access_studio: feature_flags.has_flag(DbUserFeatureFlag::Studio),
          maybe_feature_flags: feature_flags.clone_flags()
              .iter()
              .map(|f| user_feature_flag_to_api(f))
              .collect(),
