//! Tests for omni-gen against the Seedream 4 model.
//!
//! Seedream 4 is multi-function: text-to-image and image-to-image (edit).
//!
//! Artcraft-tier pricing: flat 3¢/image regardless of size, quality, or mode.
//!
//! The Fal execution plan maps aspect ratios to Seedream-native image sizes:
//!   Square, SquareHd, LandscapeFourThree, LandscapeSixteenNine,
//!   PortraitFourThree, PortraitSixteenNine, Auto, Auto2k, Auto4k.
//! Auto in t2i mode falls back to Square; in edit mode it passes through.
...
mod tests {
  ...
  use crate::http_server::endpoints::omni_gen::generate::image::pipeline_v1::distill_image_request::{
  ...
  fn make_request(
    ...
    OmniGenImageCostAndGenerateRequest {
      ...
      model: Some(CommonImageModel::Seedream4),
  ...
  fn distill_text(request: &OmniGenImageCostAndGenerateRequest) -> DistilledImageRequest {
    ...
    distill_image_request(request, Some(&empty))
      .expect("distill_image_request should succeed for Seedream 4 (text)")
  }
  ...
  fn distill_edit(
    ...
    distill_image_request(request, Some(hydration))
      .expect("distill_image_request should succeed for Seedream 4 (edit)")
  }
  ...
  mod cost {
    ...
    mod text {
      ...
      #[test]
      fn default_batch_costs_3_cents() {
        assert_eq!(cost_for_batch(None), 3);
      }

      #[test]
      fn batch_of_one_costs_3() { assert_eq!(cost_for_batch(Some(1)), 3); }

      #[test]
      fn batch_of_two_costs_6() { assert_eq!(cost_for_batch(Some(2)), 6); }

      #[test]
      fn batch_of_three_costs_9() { assert_eq!(cost_for_batch(Some(3)), 9); }

      #[test]
      fn batch_of_four_costs_12() { assert_eq!(cost_for_batch(Some(4)), 12); }

      #[test]
      fn batch_above_max_clamps_to_12() { assert_eq!(cost_for_batch(Some(7)), 12); }
      ...
      fn cost_is_independent_of_aspect_ratio() {
        ...
        for ar in ars {
          ...
          assert_eq!(
            distill_text(&request).cost.cost_in_usd_cents.unwrap(), 6,
            "expected 6¢ regardless of aspect ratio {:?}", ar,
          );
        }
      }
    ...
    mod edit {
      ...
      #[test]
      fn default_batch_costs_3() { assert_eq!(cost_edit(None, 1), 3); }

      #[test]
      fn batch_of_one_costs_3() { assert_eq!(cost_edit(Some(1), 1), 3); }

      #[test]
      fn batch_of_two_costs_6() { assert_eq!(cost_edit(Some(2), 1), 6); }

      #[test]
      fn batch_of_four_costs_12() { assert_eq!(cost_edit(Some(4), 1), 12); }

      #[test]
      fn batch_above_max_clamps_to_12() { assert_eq!(cost_edit(Some(9), 1), 12); }

      #[test]
      fn cost_independent_of_input_image_count() {
        for num_refs in [1usize, 2, 3, 5] {
          assert_eq!(cost_edit(Some(2), num_refs), 6, "expected 6¢ regardless of {} input images", num_refs);
        }
      }
  ...
  mod plan {
    ...
    use artcraft_router::generate::generate_image::plan::fal::plan_generate_image_fal_seedream_4::{
      FalSeedream4ImageSize, FalSeedream4NumImages, PlanFalSeedream4,
    };

    fn with_text_plan<F: FnOnce(&PlanFalSeedream4)>(
      request: &OmniGenImageCostAndGenerateRequest,
      assertion: F,
    ) {
      let distilled = distill_text(request);
      match distilled.plan() {
        ImageGenerationPlan::FalSeedream4(plan) => assertion(plan),
        other => panic!("expected FalSeedream4, got {:?}", other),
      }
    }

    fn with_edit_plan<F: FnOnce(&PlanFalSeedream4)>(
      request: &OmniGenImageCostAndGenerateRequest,
      hydration: &HashMap<MediaFileToken, Url>,
      assertion: F,
    ) {
    ...
    mod text {
      ...
      fn square_yields_square() {
        with_text_plan(&make_request(Some("p"), Some(CommonAspectRatio::Square), Some(1), None), |plan| {
          assert!(matches!(plan.image_size, Some(FalSeedream4ImageSize::Square)));
        });
      }
      ...
      fn square_hd_yields_square_hd() {
        with_text_plan(&make_request(Some("p"), Some(CommonAspectRatio::SquareHd), Some(1), None), |plan| {
          assert!(matches!(plan.image_size, Some(FalSeedream4ImageSize::SquareHd)));
        });
      }
      ...
      fn wide_16x9_yields_landscape_sixteen_nine() {
        with_text_plan(&make_request(Some("p"), Some(CommonAspectRatio::WideSixteenByNine), Some(1), None), |plan| {
          assert!(matches!(plan.image_size, Some(FalSeedream4ImageSize::LandscapeSixteenNine)));
        });
      }
      ...
      fn wide_4x3_yields_landscape_four_three() {
        with_text_plan(&make_request(Some("p"), Some(CommonAspectRatio::WideFourByThree), Some(1), None), |plan| {
          assert!(matches!(plan.image_size, Some(FalSeedream4ImageSize::LandscapeFourThree)));
        });
      }
      ...
      fn tall_9x16_yields_portrait_sixteen_nine() {
        with_text_plan(&make_request(Some("p"), Some(CommonAspectRatio::TallNineBySixteen), Some(1), None), |plan| {
          assert!(matches!(plan.image_size, Some(FalSeedream4ImageSize::PortraitSixteenNine)));
        });
      }
      ...
      fn tall_3x4_yields_portrait_four_three() {
        with_text_plan(&make_request(Some("p"), Some(CommonAspectRatio::TallThreeByFour), Some(1), None), |plan| {
          assert!(matches!(plan.image_size, Some(FalSeedream4ImageSize::PortraitFourThree)));
        });
      }

      // ── Image size: Auto variants ─────────────────────────────────────────

      #[test]
      fn auto_in_text_mode_yields_square() {
      ...
      fn auto_2k_yields_auto_2k() {
        with_text_plan(&make_request(Some("p"), Some(CommonAspectRatio::Auto2k), Some(1), None), |plan| {
          assert!(matches!(plan.image_size, Some(FalSeedream4ImageSize::Auto2k)));
        });
      }
      ...
      fn auto_4k_yields_auto_4k() {
        with_text_plan(&make_request(Some("p"), Some(CommonAspectRatio::Auto4k), Some(1), None), |plan| {
          assert!(matches!(plan.image_size, Some(FalSeedream4ImageSize::Auto4k)));
        });
      }

      // ── Image size: fallbacks (unsupported → nearest under PayMoreUpgrade)
      ...
      fn default_batch_count_is_one() {
        with_text_plan(&make_request(Some("p"), None, None, None), |plan| {
          assert!(matches!(plan.num_images, FalSeedream4NumImages::One));
        });
      }
      ...
      fn batch_direct_mapping() {
        for (count, expected) in [
          (1u16, FalSeedream4NumImages::One), (2, FalSeedream4NumImages::Two),
          (3, FalSeedream4NumImages::Three), (4, FalSeedream4NumImages::Four),
        ] {
      ...
      fn batch_above_four_clamps_to_four() {
        with_text_plan(&make_request(Some("p"), None, Some(9), None), |plan| {
          assert!(matches!(plan.num_images, FalSeedream4NumImages::Four));
        });
      }
    ...
    mod edit {
      ...
      // ── Auto in edit mode yields Auto (not Square) ────────────────────────

      #[test]
      fn auto_in_edit_mode_yields_auto() {
        let (tokens, hydration) = fake_image_refs(1);
        with_edit_plan(
          &make_request(Some("p"), Some(CommonAspectRatio::Auto), Some(1), Some(tokens)),
          &hydration,
          |plan| {
            assert!(matches!(plan.image_size, Some(FalSeedream4ImageSize::Auto)));
          },
        );
      }
      ...
      fn edit_square_yields_square() {
        ...
        with_edit_plan(
          ...
          &hydration,
          |plan| { assert!(matches!(plan.image_size, Some(FalSeedream4ImageSize::Square))); },
        );
      }
      ...
      fn edit_wide_16x9_yields_landscape_sixteen_nine() {
        ...
        with_edit_plan(
          ...
          &hydration,
          |plan| { assert!(matches!(plan.image_size, Some(FalSeedream4ImageSize::LandscapeSixteenNine))); },
        );
      }
      ...
      fn edit_tall_9x16_yields_portrait_sixteen_nine() {
        ...
        with_edit_plan(
          ...
          &hydration,
          |plan| { assert!(matches!(plan.image_size, Some(FalSeedream4ImageSize::PortraitSixteenNine))); },
        );
      }
      ...
      fn edit_auto_2k() {
        ...
        with_edit_plan(
          ...
          &hydration,
          |plan| { assert!(matches!(plan.image_size, Some(FalSeedream4ImageSize::Auto2k))); },
        );
      }
      ...
      fn edit_auto_4k() {
        ...
        with_edit_plan(
          ...
          &hydration,
          |plan| { assert!(matches!(plan.image_size, Some(FalSeedream4ImageSize::Auto4k))); },
        );
      }
      ...
      fn edit_default_batch_count_is_one() {
        ...
        with_edit_plan(
          ...
          &hydration,
          |plan| { assert!(matches!(plan.num_images, FalSeedream4NumImages::One)); },
        );
      }
      ...
      fn edit_batch_of_four_yields_four() {
        ...
        with_edit_plan(
          ...
          &hydration,
          |plan| { assert!(matches!(plan.num_images, FalSeedream4NumImages::Four)); },
        );
      }
