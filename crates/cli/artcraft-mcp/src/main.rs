...
fn load_storyteller_credentials() -> Result<StorytellerCredentialSet> {
  // Production desktop app v0.32 keeps Storyteller cookies in the Tauri
  // HTTP plugin's cookie jar at ~/Library/Caches/ai.artcraft.app/.cookies.
  // The text-file path under ~/Artcraft/credentials/ is wired up in the
  // dev source but the `persist_all_to_disk()` call is commented out, so
  // shipped builds never populate those files. We try the cookie jar
  // first, then fall back to text files for forward-compat.
...
/// Read the vendored tauri-plugin-http cookie jar shipped at
/// `<app_cache_dir>/.cookies`. The file is a JSON array of cookies in
/// the `cookie_store` crate's serde format; each entry's `raw_cookie`
/// field is a `Set-Cookie`-style string. We pick out the `session` and
/// `visitor` cookies scoped to api.storyteller.ai.
///
/// macOS-only path resolution for v1 — Linux/Windows fall through to
/// the text-file loader (which itself is a no-op today).
...
impl CookieEntry {
  fn domain_is(&self, host: &str) -> bool {
    // cookie_store serializes domain as { "HostOnly": "host" } or
    // { "Suffix": "host" } or "NotPresent". Match either positive shape.
...
fn tauri_cookies_jar_path() -> Option<PathBuf> {
  ...
  // Bundle id is ai.artcraft.app — see /Applications/ArtCraft.app/Contents/Info.plist.
  // ProjectDirs::from("ai", "artcraft", "app").cache_dir() resolves to the same
  // path on macOS. Linux/Windows resolutions differ from Tauri's; left for v2.
