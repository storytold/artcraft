...
mod tests {
  ...
  mod pricing_tests {
    ...
    // hundredths. Consumer converts at 193 credits/$1; enterprise converts
    // at the bulk rate of 243 credits/$1.
    ...
    mod usd_cents {
      ...
      fn consumer_cents_480p_15s() {
        ...
        assert!((cost.usd_cents_fractional - (69225.0 / 193.0)).abs() < FLOAT_TOLERANCE);
      }

      #[test]
      fn consumer_cents_720p_4s() {
        // 368.12 credits; 36812/193 = 190.7358 cents.
        let cost = r720(4).calculate_consumer_costs();
        assert_eq!(cost.usd_cents_rounded_up, 191);
        assert_eq!(cost.usd_cents_rounded_down, 190);
        assert!((cost.usd_cents_fractional - (36812.0 / 193.0)).abs() < FLOAT_TOLERANCE);
      }

      #[test]
      fn consumer_cents_720p_30s() {
        // 2760.9 credits; 276090/193 = 1430.5181 cents.
        ...
        assert!((cost.usd_cents_fractional - (276090.0 / 193.0)).abs() < FLOAT_TOLERANCE);
      }

      #[test]
      fn enterprise_cents_480p_4s() {
        // 168.52 credits; 16852/243 = 69.3498 cents.
        let cost = r480(4).calculate_enterprise_costs();
        assert_eq!(cost.usd_cents_rounded_up, 70);
        assert_eq!(cost.usd_cents_rounded_down, 69);
        assert!((cost.usd_cents_fractional - (16852.0 / 243.0)).abs() < FLOAT_TOLERANCE);
      }

      #[test]
      fn enterprise_cents_480p_15s() {
        // 631.95 credits; 63195/243 = 260.0617 cents.
        let cost = r480(15).calculate_enterprise_costs();
        assert_eq!(cost.usd_cents_rounded_up, 261);
        assert_eq!(cost.usd_cents_rounded_down, 260);
        assert!((cost.usd_cents_fractional - (63195.0 / 243.0)).abs() < FLOAT_TOLERANCE);
      }

      #[test]
      fn enterprise_cents_720p_4s() {
        // 337.04 credits; 33704/243 = 138.6996 cents.
        ...
        assert!((cost.usd_cents_fractional - (33704.0 / 243.0)).abs() < FLOAT_TOLERANCE);
      }

      #[test]
      fn enterprise_cents_720p_30s() {
        // 2527.8 credits; 252780/243 = 1040.2469 cents.
        let cost = r720(30).calculate_enterprise_costs();
        assert_eq!(cost.usd_cents_rounded_up, 1041);
        assert_eq!(cost.usd_cents_rounded_down, 1040);
        assert!((cost.usd_cents_fractional - (252780.0 / 243.0)).abs() < FLOAT_TOLERANCE);
      }
    }
