...
fn build_public_url(upload_url: &str, host: &KinoviHost) -> Result<String, KinoviWebError> {
  let parsed = Url::parse(upload_url)
    .map_err(|err| KinoviWebClientError::UrlParseError(err))?;
  let path = parsed.path(); // e.g. "/materials/20260219/1771463564512-b14bfe90.png"
  Ok(format!("{}{}", host.cdn_base_url(), path))
}

pub async fn upload_file(args: UploadFileArgs) -> Result<UploadFileResponse, KinoviWebError> {
