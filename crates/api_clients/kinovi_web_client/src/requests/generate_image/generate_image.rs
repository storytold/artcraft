...
pub struct KinoviGenerateImageRequest {
  ...
  /// `KinoviWebClientError::InvalidRequestField` error rather than
...
pub async fn generate_image(args: GenerateImageArgs<'_>) -> Result<GenerateImageResponse, KinoviWebError> {
...
/// panicking.
fn parse_quality(raw: &str) -> Result<f32, KinoviWebClientError> {
  raw.parse::<f32>().map_err(|err| KinoviWebClientError::InvalidRequestField {
    field: "quality",
...
mod tests {
  ...
  mod wire_shape_tests {
    ...
    fn try_build_wire_json(req: KinoviGenerateImageRequest) -> Result<Value, KinoviWebClientError> {
    ...
    fn quality_with_non_numeric_string_returns_client_error() {
      ...
      // structured `KinoviWebClientError::InvalidRequestField` rather
      // than panicking.
      let req = KinoviGenerateImageRequest {
      ...
      match err {
        KinoviWebClientError::InvalidRequestField { field, raw_value, .. } => {
