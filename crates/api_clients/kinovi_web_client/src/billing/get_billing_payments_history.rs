...
) -> Result<BillingPaymentsHistoryPage, KinoviWebError> {
...
fn parse_response_body(response_body: &str) -> Result<BillingPaymentsHistoryPage, KinoviWebError> {
  let batch_response: Vec<BatchResponseItem> = serde_json::from_str(response_body)
    .map_err(|err| KinoviWebGenericApiError::SerdeResponseParseErrorWithBody(err, response_body.to_string()))?;
...
mod tests {
  ...
  mod live {
    ...
    ///   cargo test -p kinovi_web_client live_paginate_entire_payments_history -- --ignored --nocapture
    #[tokio::test]
