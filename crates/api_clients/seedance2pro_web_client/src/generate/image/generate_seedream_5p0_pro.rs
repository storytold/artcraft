...
) -> Result<Seedream5p0ProApiParams, Seedance2ProWebClientError> {
  let uploaded_urls = match &request.reference_image_urls {
    Some(urls) if urls.len() > MAX_REFERENCE_IMAGES => {
      return Err(Seedance2ProWebClientError::InvalidRequestField {
...
mod tests {
  ...
  mod reference_image_limit_tests {
    ...
    fn fifteen_references_is_rejected() {
      ...
      match result {
        Err(Seedance2ProWebClientError::InvalidRequestField { field, .. }) => {
