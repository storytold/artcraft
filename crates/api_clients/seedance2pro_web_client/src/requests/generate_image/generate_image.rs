...
use crate::error::seedance2pro_web_client_error::Seedance2ProWebClientError;
...
pub struct KinoviGenerateImageRequest {
  ...
  /// `Seedance2ProWebClientError::InvalidRequestField` error rather than
...
pub async fn generate_image(args: GenerateImageArgs<'_>) -> Result<GenerateImageResponse, Seedance2ProError> {
  ...
  let client = Client::builder()
    ...
    .build()
    .map_err(|err| Seedance2ProWebClientError::WreqClientError(err))?;
...
/// panicking.
fn parse_quality(raw: &str) -> Result<f32, Seedance2ProWebClientError> {
  raw.parse::<f32>().map_err(|err| Seedance2ProWebClientError::InvalidRequestField {
    field: "quality",
...
mod tests {
  ...
  mod wire_shape_tests {
    ...
    fn try_build_wire_json(req: KinoviGenerateImageRequest) -> Result<Value, Seedance2ProWebClientError> {
    ...
    fn quality_with_non_numeric_string_returns_client_error() {
      ...
      // structured `Seedance2ProWebClientError::InvalidRequestField` rather
      // than panicking.
      let req = KinoviGenerateImageRequest {
      ...
      match err {
        Seedance2ProWebClientError::InvalidRequestField { field, raw_value, .. } => {
