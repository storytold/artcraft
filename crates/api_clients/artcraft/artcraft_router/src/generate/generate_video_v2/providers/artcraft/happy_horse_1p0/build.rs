...
pub fn build_artcraft_happy_horse_1p0(mut builder: GenerateVideoRequestBuilder) -> Result<VideoGenerationDraftOrRequest, ArtcraftRouterError> {
  ...
  let request = OmniGenVideoCostAndGenerateRequest {
    model: Some(CommonVideoModelEnum::HappyHorse1p0),
...
fn plan_output_resolution(
  ...
  match resolution {
    ...
    Some(unsupported) => match strategy {
      ...
      RequestMismatchMitigationStrategy::PayLessDowngrade => {
        Ok(Some(CommonResolutionEnum::SevenTwentyP))
      }
    },
  }
}
