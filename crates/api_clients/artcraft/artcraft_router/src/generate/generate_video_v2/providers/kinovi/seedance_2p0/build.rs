...
pub fn build_kinovi_seedance_2p0(mut builder: GenerateVideoRequestBuilder) -> Result<VideoGenerationDraftOrRequest, ArtcraftRouterError> {
  let strategy = builder.request_mismatch_mitigation_strategy;

  let aspect_ratio = plan_aspect_ratio(builder.aspect_ratio, strategy)?;
  let resolution = builder.resolution.map(map_common_resolution_to_kinovi);
  ...
  // TODO: Fill out body
  let draft = KinoviSeedance2p0DraftState {
    aspect_ratio,
    resolution,
    batch_count,
    duration_seconds,
