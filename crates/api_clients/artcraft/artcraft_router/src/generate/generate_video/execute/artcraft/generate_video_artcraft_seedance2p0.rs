...
pub async fn execute_artcraft_seedance2p0(
  ...
  let request = Seedance2p0MultiFunctionVideoGenRequest {
    ...
    reference_video_media_tokens: plan.reference_video_media_tokens.map(|tokens| tokens.to_owned()),
    reference_audio_media_tokens: plan.reference_audio_media_tokens.map(|tokens| tokens.to_owned()),
