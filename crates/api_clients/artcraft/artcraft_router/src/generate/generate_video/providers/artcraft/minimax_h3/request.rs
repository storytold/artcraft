...
#[derive(Clone, Debug)]
pub struct ArtcraftMinimaxH3RequestState {
  pub request: OmniGenVideoCostAndGenerateRequest,
}

impl ArtcraftMinimaxH3RequestState {
...
mod tests {
  ...
  async fn reference_images_to_video() {
    let response = run_pipeline(GenerateVideoRequestBuilder {
      prompt: Some("Image 1 is the dog. The dog explores a mossy forest clearing.".to_string()),
