...
#[derive(Clone, Debug)]
pub struct FalFlux3RequestState {
  pub mode: FalFlux3Mode,
}

impl FalFlux3RequestState {
...
mod tests {
  ...
  async fn send_text_to_video_webhook() {
    let state = FalFlux3RequestState {
      mode: FalFlux3Mode::TextToVideo(Flux3TextToVideoRequest {
  ...
  async fn send_image_to_video_webhook() {
    let state = FalFlux3RequestState {
      mode: FalFlux3Mode::ImageToVideo(Flux3ImageToVideoRequest {
