...
use artcraft_api_defs::generate::video::generate_kling_2_1_pro_image_to_video::{
  GenerateKling21ProAspectRatio, GenerateKling21ProDuration,
};
use tokens::tokens::media_files::MediaFileToken;

#[derive(Debug, Clone)]
pub struct PlanArtcraftKling21Pro<'a> {
...
pub fn plan_generate_video_artcraft_kling_2_1_pro<'a>(
  ...
  let start_frame = match request.start_frame {
    ...
    None => {
      return Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption {
        field: "start_frame",
        value: "Kling 2.1 Pro requires a starting frame".to_string(),
      }))
    }
  };
  ...
  Ok(VideoGenerationPlan::ArtcraftKling21Pro(PlanArtcraftKling21Pro {
...
) -> Result<Option<GenerateKling21ProAspectRatio>, ArtcraftRouterError> {
  use GenerateKling21ProAspectRatio as Ar;
  match aspect_ratio {
    None => Ok(None),
...
fn nearest_aspect_ratio(ar: CommonAspectRatio) -> GenerateKling21ProAspectRatio {
  use GenerateKling21ProAspectRatio as Ar;
  match ar {
...
fn plan_duration(
  ...
  strategy: RequestMismatchMitigationStrategy,
) -> Result<Option<GenerateKling21ProDuration>, ArtcraftRouterError> {
  use GenerateKling21ProDuration as D;
...
impl PlanArtcraftKling21Pro<'_> {
  pub fn is_ten_seconds(&self) -> bool {
    matches!(self.duration, Some(GenerateKling21ProDuration::TenSeconds))
  }
}
