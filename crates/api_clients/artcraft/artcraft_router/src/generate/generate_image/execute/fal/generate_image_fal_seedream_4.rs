...
pub async fn execute_fal_seedream_4(
  ...
  let (webhook_response, outbound_request) = if plan.image_urls.is_empty() {
    ...
    let resp = enqueue_bytedance_seedream_v4_text_to_image_webhook(args)
      .await
      .map_err(|e| ArtcraftRouterError::Provider(ProviderError::Fal(e)))?;
    (resp, outbound)
  } else {
    let request = EnqueueBytedanceSeedreamV4EditImageRequest {
      prompt: plan.prompt.clone().unwrap_or_default(),
    ...
    let resp = enqueue_bytedance_seedream_v4_edit_image_webhook(args)
      .await
      .map_err(|e| ArtcraftRouterError::Provider(ProviderError::Fal(e)))?;
    (resp, outbound)
  };
