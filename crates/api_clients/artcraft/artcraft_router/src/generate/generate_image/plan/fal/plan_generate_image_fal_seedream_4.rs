...
use fal_client::requests::webhook::image::edit::enqueue_bytedance_seedream_v4_edit_image_webhook::{
  EnqueueBytedanceSeedreamV4EditImageNumImages, EnqueueBytedanceSeedreamV4EditImageSize,
};
...
pub enum FalSeedream4NumImages {
  One,
  Two,
  Three,
  Four,
...
/// Shared image-size shape between Seedream V4 t2i and edit (variants are identical).
#[derive(Debug, Clone, Copy)]
pub enum FalSeedream4ImageSize {
  Square,
  SquareHd,
  PortraitFourThree,
  PortraitSixteenNine,
...
) -> Result<Option<FalSeedream4ImageSize>, ArtcraftRouterError> {
  use FalSeedream4ImageSize as S;
  match aspect_ratio {
    None => Ok(None),
...
fn plan_num_images(
  ...
  strategy: RequestMismatchMitigationStrategy,
) -> Result<FalSeedream4NumImages, ArtcraftRouterError> {
  let count = image_batch_count.unwrap_or(1);
  match count {
    0 => Err(ArtcraftRouterError::Client(ClientError::UserRequestedZeroGenerations)),
    1 => Ok(FalSeedream4NumImages::One),
    2 => Ok(FalSeedream4NumImages::Two),
    3 => Ok(FalSeedream4NumImages::Three),
    4 => Ok(FalSeedream4NumImages::Four),
    _ => match strategy {
      RequestMismatchMitigationStrategy::ErrorOut => {
        Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption {
      ...
      _ => Ok(FalSeedream4NumImages::Four),
    },
  }
}

impl FalSeedream4NumImages {
  pub fn to_t2i(self) -> EnqueueBytedanceSeedreamV4TextToImageNumImages {
  ...
  pub fn to_edit(self) -> EnqueueBytedanceSeedreamV4EditImageNumImages {
...
impl FalSeedream4ImageSize {
  pub fn to_t2i(self) -> EnqueueBytedanceSeedreamV4TextToImageSize {
  ...
  pub fn to_edit(self) -> EnqueueBytedanceSeedreamV4EditImageSize {
