...
#[derive(Debug, Clone)]
pub struct PlanArtcraftFluxPro11<'a> {
  pub prompt: Option<&'a str>,
  pub aspect_ratio: Option<GenerateFluxPro11TextToImageAspectRatio>,
  pub num_images: GenerateFluxPro11TextToImageNumImages,
  pub idempotency_token: String,
}

pub fn plan_generate_image_artcraft_flux_pro_1p1<'a>(
  request: &'a GenerateImageRequest<'a>,
) -> Result<PlanArtcraftFluxPro11<'a>, ArtcraftRouterError> {
  ...
  if request.image_inputs.is_some() {
    if let Some(GenerationModeMismatchStrategy::AbortGeneration) = request.generation_mode_mismatch_strategy {
      return Err(ArtcraftRouterError::Client(ClientError::ModelDoesNotSupportOption {
        field: "image_inputs",
        value: "Flux Pro 1.1 is a text-to-image model and does not support image editing".to_string(),
      }));
    }
...
// Flux Pro 1.1 supported aspect ratios (no SquareHd):
//   Square, LandscapeFourByThree, LandscapeSixteenByNine,
//   PortraitThreeByFour, PortraitNineBySixteen
fn plan_aspect_ratio(
  aspect_ratio: Option<CommonAspectRatio>,
  strategy: RequestMismatchMitigationStrategy,
) -> Result<Option<GenerateFluxPro11TextToImageAspectRatio>, ArtcraftRouterError> {
  use GenerateFluxPro11TextToImageAspectRatio as FlAr;
  match aspect_ratio {
    None => Ok(None),
...
fn nearest_aspect_ratio(aspect_ratio: CommonAspectRatio) -> GenerateFluxPro11TextToImageAspectRatio {
  use GenerateFluxPro11TextToImageAspectRatio as FlAr;
  match aspect_ratio {
...
fn plan_num_images(
  ...
  strategy: RequestMismatchMitigationStrategy,
) -> Result<GenerateFluxPro11TextToImageNumImages, ArtcraftRouterError> {
  use GenerateFluxPro11TextToImageNumImages as FlN;
...
mod tests {
  ...
  use artcraft_api_defs::generate::image::text::generate_flux_pro_11_text_to_image::{
    ...
    GenerateFluxPro11TextToImageNumImages as FlN,
  };
