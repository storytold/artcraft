...
/// Thread-safe bounded buffer of request samples. Cheap to clone (Arc inside).
#[derive(Clone)]
...
impl SampleQueue {
  ...
  pub fn push(&self, sample: RequestSample) {
    ...
    if guard.len() >= self.max_samples {
      ...
      // Avoid log floods: only warn on the first overflow per drain cycle.
      // (We rely on the worker logging at flush time.)
    }
    guard.push(sample);
  }

  /// Atomically take all currently-buffered samples.
  pub fn drain(&self) -> Vec<RequestSample> {
    let mut guard = match self.inner.lock() {
      Ok(g) => g,
      Err(poison) => {
...
mod tests {
  ...
  fn sample(route: &str) -> RequestSample {
    RequestSample {
  ...
  fn full_queue_drops_oldest() {
    ...
    q.push(sample("/old"));
    q.push(sample("/older"));
    q.push(sample("/oldest"));
    q.push(sample("/new"));
    let drained = q.drain();
    assert_eq!(drained.len(), 3);
    assert!(drained.iter().any(|s| s.route == "/new"));
  }
}
